...
`EventOrAddressRef` is the one representation of a pointer to a parent event (reply roots/parents, repost / reaction / highlight targets): a discriminated union `{ type: "event", id: EventId } | { type: "address", address: AddressableEventRef }`. `formatEventOrAddressRef(ref)` gives its canonical wire and storage form (the hex id, or `kind:pubkey:d`) and is the key to use in a `Map` / `Set`. `parseEventOrAddressRef(value)` (in `service/event-or-address-ref.ts`, since it decodes NIP-19) accepts a hex id, a `kind:pubkey:d` coordinate, or a `note1…` / `nevent1…` / `naddr1…` entity and returns `null` otherwise; `eventOrAddressRefFromTag(tag)` reads one from an `e` / `E` / `a` / `A` tag. An address always has a non-empty `d`.
...
- `transformer.ts` — `transformEvent(event)` returns a normalised view including `refs.isReply`; `refs.rootEvent` / `refs.replyToEvent` and the repost / reaction / highlight targets are `EventOrAddressRef`s, and `replyTargetRef(event)` is the ref a reply to `event` carries. **This is the ONE way to determine reply-ness** (kinds 1 and 1111 only).
- `client-message.ts` / `relay-message.ts` — `serialise{Req,Event,Close,Auth}Message(...)` produce the client→relay wire strings; `parseRelayMessage(raw)` is the inverse for relay→client messages, returning a typed `RelayMessage` union (`EVENT` / `EOSE` / `OK` / `CLOSED` / `NOTICE` / `AUTH` / `COUNT`) or `null` for anything malformed. An `EVENT` payload is validated through `parseNostrEvent` and an `OK` event id is branded, so consumers get typed, checked data rather than raw arrays.
...
- `buildRumor(event)` attaches the computed NIP-01 `id` to an unsigned event (`EventToSign`), producing the `Rumor` (a `RenderableEvent`: unsigned, but carrying `id` and `pubkey`, as NIP-59 defines it). The one place a rumor's id is attached.
- `buildDmGiftWraps({ signer, rumor, ... })` builds the gift-wrap pair (one for recipient, one for sender) for a `Rumor`, encrypting it with its `id` — returns `Result<ReadonlyArray<GiftWrapTarget>, EncryptionError>`.
- `unwrapGiftWrap(signer, event)` reverses the kind-1059 → seal → rumor wrapping, returning `Result<UnwrapResult, GiftWrapUnwrapError>`. `GiftWrapUnwrapError` is tagged `not-gift-wrap | seal-decrypt-failed | seal-malformed | seal-wrong-kind | rumor-decrypt-failed | rumor-malformed | rumor-id-mismatch | rumor-wrong-kind | rumor-pubkey-mismatch` — UI callers can surface diagnostics; bulk-feed callers wanting the hot null path do `result.success ? result.value : null`.
- `parseRumor(value)` (async) validates `value` as a rumor and returns `Promise<Rumor | null>`. A present `id` must equal the id computed from the other fields (otherwise `null`; `unwrapGiftWrap` reports it as `rumor-id-mismatch`); an absent `id` is derived, so rumors from clients that omit it still parse. `null` also for any invalid field.
...
- `GiftWrapUnwrapError` — `unwrapGiftWrap` failure; tagged `not-gift-wrap | seal-decrypt-failed | seal-malformed | seal-wrong-kind | rumor-decrypt-failed | rumor-malformed | rumor-id-mismatch | rumor-wrong-kind | rumor-pubkey-mismatch`.
