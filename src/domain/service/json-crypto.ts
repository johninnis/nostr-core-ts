...
/** JSON-serialise `value` and NIP-44 encrypt it with `cipher`; non-serialisable values fail with `json-stringify-failed`. */
export const encryptJson = async (
  ...
  const encrypted = await cipher.nip44Encrypt(pubkey, json)
...
/**
 * NIP-44 decrypt with `cipher` and JSON-parse the plaintext; empty or non-JSON payloads fail with
...
export const decryptJson = async (
  ...
  const decrypted = await cipher.nip44Decrypt(pubkey, ciphertext)
